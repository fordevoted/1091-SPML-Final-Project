...
train_loader = torch.utils.data.DataLoader(
    dset.ImageFolder(r'D:\temp\train', transforms.Compose([
...
def train(epoch, patch, patch_shape):
    ...
    for batch_idx, (data, labels) in enumerate(train_loader):
        ...
        adv_x, mask, patch = myAttack(data, patch, mask)
        output = netClassifier(adv_x)
        adv_label = output.data.max(1)[1][0]
        ori_label = labels.data[0]
        # print("adv_label: {}, original_label:{}".format(adv_label, ori_label))
        if adv_label == target and F.softmax(output).data[0][target] > 0.80:
            success += 1
            ...
            if plot_all == 1:
                ...
                vutils.save_image(adv_x.data, "./%s/%d_%d_adversarial.png" % (opt.outf, batch_idx, adv_label),
                                  normalize=True)
                # vutils.save_image(patch.data, "./%s/%d_%d_patch.png" % (opt.outf, batch_idx, adv_label),
                #                   normalize=True)
        vutils.save_image(adv_x.data, "./%s_test/%d_%d_adversarial.png" % (opt.outf, batch_idx, adv_label),
                          normalize=True)
...
def test(epoch, patch, patch_shape):
    ...
    try:
        print("Test Success: {:.3f}".format(success / total+1e-12))
    except ZeroDivisionError as e:
        print("Test Success: {}, Total: {}".format(success, total))
        pass
def print_grad(grad):
    print(grad)
    return grad
...
def attack(x, patch, mask):
    ...
    count = 0
    L_cc = L_color_consistancy()
    L_tv = L_TV()
    L_dark_channel = L_Dark_Channel()
    L_spa = L_SPA()
    while conf_target > target_prob:
        ...
        adv_in = adv_x + adv_patch
        adv_out = F.log_softmax(netClassifier(adv_in))

        adv_out_probs, adv_out_labels = adv_out.max(1)
        # print("adv before: {}", adv_out[0][target], end='\t')
        Loss = 1 * (-adv_out[0][target]) #+ 5 * L_dark_channel(adv_patch, 1) # + 10 * L_tv(adv_patch)
        ...
        out = F.softmax(netClassifier(adv_in))
        target_prob = out.data[0][target]
        # print(
        #     "loss check: advout: {} Ldc: {} Lcc: {} Ltv: {} Lspa: {}".format((-adv_out[0][target]).cpu().detach().numpy(),
        #                                                                      2*L_dark_channel(adv_patch, 1).cpu().detach().numpy(),
        #                                                                      L_cc(adv_patch).cpu().detach().numpy(),
        #                                                                      2*L_tv(adv_patch).cpu().detach().numpy(),
        #                                                                      L_spa(adv_patch)), target_prob)
...
def myAttack(x, patch, mask):
    ...
    # adv_patch = adv_patch.reshape(3, int(np.sqrt(np.divide(adv_patch.size(),3))[0], int(np.sqrt(np.divide(adv_patch.size(),3))[0])))
    count = 0
    L_cc = L_color_consistancy()
    L_tv = L_TV()
    adv_in = adv_x
    while conf_target > target_prob:
        ...
        Loss = 1 * -adv_out[0][target]  # + 0.1 * L_cc(adv_patch) + 0.1 * L_tv(adv_patch)
        Loss.backward()
        ...
        patch[patch != 0] -=  5 * adv_grad

        adv_in = torch.mul((1 - mask), x) + torch.mul(mask, patch)
        adv_in = torch.clamp(adv_in, min_out, max_out)
        adv_patch = torch.mul(mask, patch)
        adv_patch = adv_patch[adv_patch != 0]
